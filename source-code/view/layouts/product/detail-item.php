...
<div class="detail-item w-85 mx-auto">
  ...
  <div class="col w-50 m-3">
    ...
    $sizes_of_productColor = $product_model->get_sizes_of_productColor($product_id, $color['color_id']);
    $size_id_0 = $sizes_of_productColor[0]['size_id'];
    ?>
          <a class="w-15 text-decoration-none text-dark text-center thumbnail" href="../app/index.php?detail_item&&product_id=<?php echo $product_id; ?>&&color_id=<?php echo $color['color_id']; ?>&&size_id=<?php echo $size_id_0; ?>">
          <img class='vw-6 w-95 <?php if ($color['color_id'] == $color_id) {
    ...
    <div class="shoe-size mt-2">
      ...
      <?php $sizes_of_productColor = $product_model->get_sizes_of_productColor($product_id, $color_id);
foreach ($sizes_of_productColor as $size) {
    ?>
    ...
    <div class="shoe-count mt-4 d-flex">
      ...
      <input type="number" onchange="change_quantity(<?php echo $quantity ?>)" id="quantity" 
        <?php if ($quantity < 1) echo "disabled" ?>
        class="ms-3 w-25 text-center form-control p-1" 
        min='<?php echo ($quantity > 0) ? 1 : 0 ?>' 
        value="<?php echo ($quantity > 0) ? 1 : 0 ?>" max='<?php echo $quantity; ?>'>
    </div>

    <div class="order">
      <button <?php if ($quantity < 1) echo "disabled" ?> type="button" id="buy-btn" class="w-50 fs-5 btn btn-dark m-5" onclick="add_item_handler(<?php echo $quantity; ?>, <?php echo $user_id ?>)">Đặt mua</button>
    </div>
...
<div class="modal" id="invalid-input" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
